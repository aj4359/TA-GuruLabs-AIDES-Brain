...
export function decodeKCode(code = '') {
  ...
  return {
    ...
    explanation: 'A K code is used when coding deductions exceed coding allowances. Instead of giving tax-free pay, payroll effectively adds an amount to taxable pay. The exact live code depends on HMRC coding inputs and should be checked against the P2/code breakdown.',
...
export function buildKCodeIYAStory(input = {}) {
  ...
  const scenes = [
    ...
    ...(statedIYA > 0 ? [{id:'IYA',title:'What the IYA means',evidenceClass:'OBSERVED',detail:`HMRC has identified £${statedIYA.toFixed(2)} of extra tax to collect during the current tax year. The related In Year Adjustment Restriction (IYAR) changes the code so the extra tax can be collected over the remaining weeks or months.`}] : []),
    ...
    ...(statedIYA > 0 && remainingPeriods > 0 ? [{id:'SPREAD',title:'Payday road',evidenceClass:'ILLUSTRATIVE',detail:`An equal-spread picture would be about £${(statedIYA/remainingPeriods).toFixed(2)} across ${remainingPeriods} remaining pay periods. This is only a visual aid, not HMRC's actual IYAR calculation.`}] : []),
  ...
  return {
    version:'25', taxCode, previousCode, kCode:k, iyaAmount:statedIYA, previousYearUnderpayment, basisMarker, reasons, scenes,
    customerSummary: k
